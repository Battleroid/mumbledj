...
RUN apk add --update ca-certificates go ffmpeg make build-base opus-dev python aria2 git
RUN apk upgrade

RUN wget https://yt-dl.org/downloads/latest/youtube-dl -O /bin/youtube-dl && chmod a+x /bin/youtube-dl

COPY . /src/github.com/RichardNysater/mumbledj
COPY config.yaml /root/.config/mumbledj/config.yaml

WORKDIR /src/github.com/RichardNysater/mumbledj

RUN go get -u -v github.com/RichardNysater/mumbledj
RUN make
RUN make install
